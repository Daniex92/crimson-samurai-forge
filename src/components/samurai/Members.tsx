...
const members = [
  { name: "BLACK_WHITE", tag: "@kuro_san", role: "IGL · Líder", img: p1, kanji: "頭" },
  { name: "MAST3R", tag: "@akari_98", role: "Sniper · Larga Distancia", img: p2, kanji: "弓" },
      ...
      <div className="mx-auto max-w-7xl px-6 lg:px-10">
        <div className="mb-16 flex flex-col items-center text-center">
          <div className="mb-4 flex items-center gap-3 font-display text-xs uppercase tracking-[0.4em] text-blood">
            <span className="h-px w-8 bg-blood" /> 02 — Los Guerreros <span className="h-px w-8 bg-blood" />
          </div>
          <h2 className="font-display text-4xl font-bold uppercase text-foreground sm:text-5xl">
            La <span className="text-blood text-glow-soft">Hermandad</span>
          </h2>
          <p className="mt-4 max-w-xl text-muted-foreground">
            Cada guerrero elegido por prueba. Cada nombre grabado en victoria.
          </p>
        </div>
          ...
          {members.map((m, i) => (
            ...
            >
              <div className="relative aspect-[3/4] overflow-hidden">
                <img
                  src={m.img}
                  alt={`${m.name}, ${m.role} de SAMURAI-BS`}
                  loading="lazy"
                  width={768}
                  height={1024}
